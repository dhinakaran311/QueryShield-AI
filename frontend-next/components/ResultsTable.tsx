        ...
        <table className="w-full text-sm">
              ...
              <tr key={hg.id} className="border-b border-slate-700 bg-slate-800">
                {hg.headers.map((header) => (
                  <th
                    ...
                    className="px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider cursor-pointer select-none hover:text-white"
                  >
          ...
          <tbody>
            {table.getRowModel().rows.map((row, i) => (
              <tr
                key={row.id}
                className={`border-b border-slate-800 ${i % 2 === 0 ? "bg-slate-900" : "bg-slate-900/50"} hover:bg-slate-800 transition-colors`}
              >
